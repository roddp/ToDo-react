...
function App() {
  ...
  return (
    <div className="wrapper">
      ...
      {!hideTasks && toDo.map((task) => (<Task
        key={task.id}
        todo={task}
        deleteTodo={delTodo}
        setCompleted={updateCompleted} />))}
      <Button className="endButtons" onClick={handleHide}>Hide all</Button>
      <Button className="endButtons" onClick={deleteDone}>Delete Done</Button>
    </div>
  );
}
